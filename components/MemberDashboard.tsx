    ...
    return (
        <div className="w-full max-w-6xl animate-fade-in">
             <h2 className="text-3xl md:text-4xl font-bold text-text-primary mb-6 text-center">
                Welcome back, <span className="text-brand-green-light">{data.name}</span>!
             </h2>
                     ...
                     <DashboardCard title="Your Order History" icon={<HistoryIcon />}>
                       <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                            {data.orderHistory.map(order => (
                                <div key={order.id} className="flex justify-between items-center p-4 bg-background rounded-lg border border-border-color hover:border-brand-green-light/50 transition-colors duration-300">
                                    <div>
                                        <p className="font-bold text-text-primary">{order.dishName}</p>
                                        <p className="text-sm text-text-secondary">from {order.restaurant}</p>
                                    </div>
                                    <p className="text-sm font-medium text-text-secondary bg-white/5 px-2 py-1 rounded-md">{order.date}</p>
                                </div>
                            ))}
                       </div>
                    </DashboardCard>
